...
router = APIRouter(prefix="/multi-symbol-backtests", tags=["multi-symbol-backtests"])
logger = structlog.get_logger("api.multi_symbol_backtests")


@router.get("", response_model=MultiSymbolRunListResponse)
def list_multi_symbol_backtests(
    ...
    get_rate_limiter().check(
        bucket="multi_symbol_backtests:read",
        actor_key=str(user.id),
        limit=settings.backtest_read_rate_limit,
        window_seconds=settings.rate_limit_window_seconds,
    )
    with MultiSymbolBacktestService(db) as service:
        return service.list_runs(user, limit=limit, offset=offset, cursor=cursor)


@router.post("", response_model=MultiSymbolRunDetailResponse, status_code=status.HTTP_202_ACCEPTED)
def create_multi_symbol_backtest(
    ...
    if not settings.feature_multi_symbol_backtests_enabled:
        raise FeatureLockedError("Multi-symbol backtesting is temporarily disabled.", required_tier="free")
    get_rate_limiter().check(
        bucket="multi_symbol_backtests:create",
        actor_key=str(user.id),
...
@router.get("/{run_id}", response_model=MultiSymbolRunDetailResponse)
def get_multi_symbol_backtest(
    run_id: UUID,
    user: User = Depends(get_current_user_readonly),
    db: Session = Depends(get_db),
    settings: Settings = Depends(get_settings),
...
@router.get("/{run_id}/status", response_model=MultiSymbolRunStatusResponse)
def get_multi_symbol_backtest_status(
    run_id: UUID,
    user: User = Depends(get_current_user_readonly),
    db: Session = Depends(get_db),
    settings: Settings = Depends(get_settings),
...
def delete_multi_symbol_backtest(
    run_id: UUID,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
    settings: Settings = Depends(get_settings),
    ...
    get_rate_limiter().check(
        bucket="multi_symbol_backtests:delete",
        actor_key=str(user.id),
...
@router.post("/{run_id}/cancel", response_model=MultiSymbolRunStatusResponse)
def cancel_multi_symbol_backtest(
    run_id: UUID,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
    settings: Settings = Depends(get_settings),
