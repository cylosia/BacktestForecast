...
import type { MultiStepRunHistoryItemResponse } from "@/lib/api/multi-workflow-types";
...
export function SelectableMultiStepHistoryList({
  items,
  comparisonLimit,
}: {
  ...
  function handleCompare() {
    ...
    router.push(`/app/multi-step-backtests/compare?ids=${encodeURIComponent(Array.from(selected).join(","))}`);
  }

  if (items.length === 0) {
    return (
      <div className="rounded-xl border border-dashed p-10 text-center">
        <p className="text-base font-medium">No multi-step runs yet</p>
        <p className="mt-2 text-sm text-muted-foreground">Create your first staged workflow to populate history.</p>
      </div>
    );
  }
              ...
              <TableRow key={item.id} className={isSelected ? "bg-primary/5" : undefined}>
                  ...
                  <TableCell>
                    {isSucceeded ? (
                      <input
                        ...
                        aria-label={`Select ${item.name ?? `${item.symbol} ${item.workflow_type}`} for comparison`}
                ...
                <TableCell>
                  <div className="space-y-1">
                    <Link className="font-medium hover:text-primary" href={`/app/multi-step-backtests/${item.id}`}>
                      {item.name ?? `${item.symbol} · ${item.workflow_type}`}
                    </Link>
                    <p className="text-xs text-muted-foreground">{item.symbol} · {item.workflow_type}</p>
                  </div>
                </TableCell>
