...
const ADVANCED_STRATEGIES = ADVANCED_STRATEGY_GROUPS.flatMap(
  (g) => g.strategies,
);
...
export function ScannerForm({
  ...
  const defaultMode: ScannerMode = scannerModes.includes("advanced")
    ? "advanced"
    : "basic";
  ...
  useEffect(() => {
    const allowed: Set<string> = new Set(
      (form.mode === "advanced" ? ADVANCED_STRATEGIES : BASIC_STRATEGIES).map(
        (s) => s.value,
      ),
    );
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      const effectiveStrategies = new Set(
        [...selectedStrategies].filter((v) => allowed.has(v)),
      );
      ...
      if (form.rsiEnabled) {
        ...
        if (
          !Number.isFinite(rsiThresholdNum) ||
          rsiThresholdNum < 0 ||
          rsiThresholdNum > 100
        ) {
          setStatus("error");
      ...
      const payload: CreateScannerJobRequest = {
        ...
        rule_sets: [
          {
            name: form.ruleSetName.trim() || "Default",
  ...
  return (
    <form
      ...
      aria-label="Scanner configuration"
    >
      {errorMessage && isPlanLimitError(errorCode) ? (
        ...
        <div
          role="alert"
          className="rounded-xl border border-destructive/40 bg-destructive/5 p-4 text-sm text-destructive whitespace-pre-line"
        >
          {errorMessage}
        </div>
      ) : null}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Define the symbols, strategies, and timeframe for this scanner job.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input
                id="scanName"
                maxLength={120}
                placeholder="e.g. Weekly momentum scan"
                value={form.name}
            ...
            <div className="space-y-2">
              ...
              <Select
                id="scanMode"
                value={form.mode}
                options={scannerModes.map((m) => ({
                  value: m,
                  label: m === "basic" ? "Basic" : "Advanced",
                }))}
                onChange={(e) =>
                  setField("mode", e.target.value as ScannerMode)
                }
              />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => setField("symbolsText", e.target.value)}
            />
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Define at least one named rule set with entry conditions.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => setField("ruleSetName", e.target.value)}
            />
          </div>
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Trigger entries when RSI crosses a threshold.
                </p>
              </div>
              <label className="inline-flex items-center gap-2 text-sm font-medium">
                <input
                  type="checkbox"
                  className="h-4 w-4 rounded border-input"
                  checked={form.rsiEnabled}
                  onChange={(e) => setField("rsiEnabled", e.target.checked)}
                />
                Enabled
              </label>
            </div>
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-3">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setField("rsiThreshold", e.target.value)}
                  />
                </div>
                <div className="space-y-2">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-6">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setField("targetDte", e.target.value)}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setField("dteTolerance", e.target.value)}
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setField("maxHolding", e.target.value)}
              />
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              className="max-w-32"
              value={form.maxRecs}
              onChange={(e) => setField("maxRecs", e.target.value)}
            />
            <p className="text-xs text-muted-foreground">
              1 to {getScannerLimits(planTier, form.mode).maxRecommendations}.
              Top-ranked combinations returned after evaluation.
            </p>
          </div>
        </CardContent>
      </Card>
