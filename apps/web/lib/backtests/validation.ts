...
import { daysAgoET, marketDateTodayET } from "@/lib/utils";
import {
  ACCOUNT_SIZE_MAX,
  ACCOUNT_SIZE_MIN,
  DTE_TOLERANCE_MAX,
...
export function validateBacktestForm(values: BacktestFormValues): {
  ...
  if (!errors.endDate) {
    const marketDateInNewYork = marketDateTodayET();
    if (values.endDate > marketDateInNewYork) {
      errors.endDate = "End date cannot be after the latest market date (America/New_York time).";
    }
  }
