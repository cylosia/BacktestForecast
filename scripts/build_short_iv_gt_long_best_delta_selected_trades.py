...
DEFAULT_OUTPUT_TRADES_CSV = LOGS / "short_iv_gt_long_best_delta_selected_trades.csv"
DEFAULT_OUTPUT_SUMMARY_CSV = LOGS / "short_iv_gt_long_best_delta_selected_summary.csv"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build a best-delta selected-trades CSV directly from one or more delta-grid trade ledgers. "
            "Each symbol/prediction/date keeps only the row matching the symbol's best delta."
        )
    )
    parser.add_argument(
        "--best-delta-csv",
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="One or more delta-grid trade ledgers to source rows from.",
    )
    parser.add_argument(
        "--entry-dates",
        nargs="*",
        default=None,
        help="Optional explicit entry dates to include. Defaults to all entry dates found in the grid rows.",
    )
    parser.add_argument(
        "--output-trades-csv",
        type=Path,
        default=DEFAULT_OUTPUT_TRADES_CSV,
        help="Output CSV for selected best-delta trades.",
    )
    parser.add_argument(
        "--output-summary-csv",
        type=Path,
        default=DEFAULT_OUTPUT_SUMMARY_CSV,
        help="Output CSV summarizing counts and gaps by entry date/prediction.",
    )
    return parser
...
def _build_selected_row(grid_row: dict[str, str]) -> dict[str, object]:
    return {
        ...
        "source": "best_delta_grid_selected",
    }
...
def main() -> int:
    ...
    if not grid_index:
        raise SystemExit("No usable grid rows were found.")

    if args.entry_dates:
        entry_dates = list(dict.fromkeys(args.entry_dates))
    else:
        entry_dates = sorted({entry_date for entry_date, _symbol in available_predictions_by_date_symbol})
        ...
        for prediction in ("up", "abstain"):
            ...
            for symbol in symbols_for_date:
                ...
                if grid_row is None:
                    ...
                    continue
                selected_rows.append(_build_selected_row(grid_row))
                added_count += 1
    ...
    if not selected_rows:
        raise SystemExit("No selected rows were built from the requested entry dates.")
    ...
    print(f"Added {len(selected_rows)} rows across {len(entry_dates)} entry dates.")
    return 0
