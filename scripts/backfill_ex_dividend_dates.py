...
class DividendBackfillStatusSnapshot(TypedDict, total=False):
    started_at: str
    command: str
    launcher_pid: int
    python_pid: int
...
def _iter_symbol_chunks(
    *,
    ...
    end_date: date,
    chunk_mode: str,
) -> Iterator[DividendBackfillChunk]:
    cursor = start_date
    while cursor <= end_date:
        ...
        if chunk_end > end_date:
            chunk_end = end_date
        yield DividendBackfillChunk(symbol=symbol, start_date=cursor, end_date=chunk_end)
...
def _build_chunks(
    *,
    ...
    end_date: date,
    chunk_mode: str,
) -> list[DividendBackfillChunk]:
    chunks: list[DividendBackfillChunk] = []
...
def _sync_chunk(
    ...
    *,
    ...
    try:
        dividends = client.list_ex_dividend_records(chunk.symbol, chunk.start_date, chunk.end_date)
    ...
    stored_count = len(rows) if dry_run or store is None else store.upsert_ex_dividend_dates(rows)
    return ChunkSyncResult(
        chunk=chunk,
...
def _read_status_snapshot(snapshot_path: str | None) -> DividendBackfillStatusSnapshot:
    ...
    try:
        ...
        raise ValueError(f"Unable to parse dividend backfill status file '{snapshot_path}': {exc}") from exc
    if not isinstance(payload, dict):
        raise ValueError(f"Dividend backfill status file '{snapshot_path}' does not contain a JSON object.")
    return payload
...
def _parse_chunk_checkpoints(raw: object) -> dict[str, ChunkCheckpointPayload]:
    ...
    for raw_key, payload in raw.items():
        ...
        checkpoints[raw_key] = {
            ...
            "chunk_end": chunk_end,
            "dividend_count": int(payload.get("dividend_count", 0)),
...
def _checkpoint_row_totals(checkpoints: dict[str, ChunkCheckpointPayload]) -> tuple[int, int]:
    dividend_total = sum(payload["dividend_count"] for payload in checkpoints.values())
    stored_total = sum(payload["stored_count"] for payload in checkpoints.values())
    return dividend_total, stored_total
...
def _load_resume_checkpoint(
    *,
    ...
    if not status_path:
        raise ValueError("--resume requires --status-path or BACKTESTFORECAST_DIVIDEND_BACKFILL_STATUS_PATH.")
    ...
    if existing_signature is not None and existing_signature != run_signature:
        raise ValueError("Existing status checkpoint belongs to a different dividend backfill window or symbol set; refusing to resume.")
...
def _write_status_snapshot(
    ...
    **fields: object,
    ...
    for attempt in range(1, max_attempts + 1):
        payload: DividendBackfillStatusSnapshot = {}
        if path.exists():
            try:
                payload = json.loads(path.read_text(encoding="utf-8"))
            except Exception:
...
def _print_chunk_result(result: ChunkSyncResult, *, processed: int, total: int, dry_run: bool) -> None:
    ...
    if result.error is None:
        print(
            ...
            f"dividends={result.dividend_count} {action}={result.stored_count}",
            flush=True,
        )
        return
    print(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill historical ex-dividend dates from Massive into Postgres by symbol/date-range chunks."
    )
    ...
    parser.add_argument("--dry-run", action="store_true", help="Fetch and count dividends without writing to Postgres.")
    ...
    status_path = (
        args.status_path
        or os.getenv("BACKTESTFORECAST_DIVIDEND_BACKFILL_STATUS_PATH")
    ...
    requested_symbols = sorted(_parse_symbols_csv(args.symbols))
    resolved_symbols: list[str] = []
    chunks: list[DividendBackfillChunk] = []
    chunk_checkpoints: dict[str, ChunkCheckpointPayload] = {}
    ...
    try:
        ...
        if not resolved_symbols:
            raise ValueError("No symbols were resolved for the requested dividend backfill window.")
        ...
        if not chunks:
            raise ValueError("No chunks were generated for the requested dividend backfill window.")
        ...
        progress_fields = _progress_snapshot_fields(chunks, chunk_checkpoints)
        completed_dividend_rows, completed_stored_rows = _checkpoint_row_totals(chunk_checkpoints)
        ...
        _write_status_snapshot(
            ...
            completed_dividend_rows=completed_dividend_rows,
