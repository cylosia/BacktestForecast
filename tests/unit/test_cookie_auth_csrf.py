...
def test_cookie_auth_requires_xrw_header():
    ...
    assert "POST" in source and "PATCH" in source and "DELETE" in source, (
        "CSRF check must cover POST, PATCH, and DELETE methods"
    )
...
def test_cookie_auth_checks_origin():
    ...
    assert "web_cors_origins" in source, (
        "Origin must be checked against configured CORS origins"
    )
