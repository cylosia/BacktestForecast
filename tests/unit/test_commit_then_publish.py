...
def test_commit_then_publish_commits_and_publishes():
    ...
    session = MagicMock()
    job_id = uuid4()
    with patch("apps.worker.app.tasks.publish_job_status") as mock_publish:
...
def test_commit_then_publish_tolerates_publish_failure():
    ...
    session = MagicMock()
    job_id = uuid4()
    with patch("apps.worker.app.tasks.publish_job_status", side_effect=Exception("Redis down")):
